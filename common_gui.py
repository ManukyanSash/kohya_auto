...
def set_pretrained_model_name_or_path_input(
    model_listt, pretrained_model_name_or_path, pretrained_model_name_or_path_file, pretrained_model_name_or_path_folder, v2, v_parameterization, sdxl
):
    if isinstance(model_listt, list):
        models_final_list = list()
        pretrained_model_name_or_path_str = dict()
        pretrained_model_name_or_path_file_str = dict()
        pretrained_model_name_or_path_folder_str = dict()
        v2_str = dict()
        v_parameterization_str = dict()
        sdxl_str = dict()
        for model_list in model_listt:
        # Check if the given model_list is in the list of SDXL models
            if str(model_list) in SDXL_MODELS:
                ...
                models_final_list.append(model_list)
                pretrained_model_name_or_path_str.update({model_list: pretrained_model_name_or_path})
                pretrained_model_name_or_path_file_str.update(pretrained_model_name_or_path_file)
                pretrained_model_name_or_path_folder_str.update(pretrained_model_name_or_path_folder) 
                v2_str.update(v2)
                v_parameterization_str.update(v_parameterization)
                sdxl_str.update(sdxl)
        ...
        if models_final_list:
            return models_final_list, pretrained_model_name_or_path_str, pretrained_model_name_or_path_file_str, pretrained_model_name_or_path_folder_str, v2_str, v_parameterization_str, sdxl_str
    else:
        model_list = model_listt
        # Check if the given model_list is in the list of SDXL models
        if str(model_list) in SDXL_MODELS:
            log.info('SDXL model selected. Setting sdxl parameters')
            v2 = gr.Checkbox.update(value=False, visible=False)
