...
def train_model(
    ...
    count = 1
    tmp_output_dir = output_dir[:-5]
    tmp_logging_dir = logging_dir[:-3]
    tmp_train_data_dir = train_data_dir[:-3]

    evaluated_models_dict = eval(pretrained_model_name_or_path)
    for model in evaluated_models_dict.keys():
        if not os.path.exists(tmp_train_data_dir + str(count) + "/img"):
            output_message(
        ...
        output_dir = tmp_output_dir + str(count) + "/model"
        logging_dir = tmp_logging_dir + str(count)+"/log"
        train_data_dir = tmp_train_data_dir + str(count) + "/img"
