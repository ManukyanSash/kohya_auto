...
def dreambooth_folder_preparation(
    ...
    for count in range(1, int(util_training_models_count)+1):
    # Create the training_dir path
        ...
        if not os.path.exists(os.path.join(util_training_dir_output, str(count), 'log')):
            os.makedirs(os.path.join(util_training_dir_output, str(count), 'log'))

        # Check if the model folder exists and create it if it doesn't
        if not os.path.exists(os.path.join(util_training_dir_output, str(count), 'model')):
            os.makedirs(os.path.join(util_training_dir_output, str(count), 'model'))
...
def gradio_dreambooth_folder_creation_tab(
    ...
    with gr.Tab('Dreambooth/LoRA Folder preparation'):
        ...
        with gr.Row():
            util_training_models_count = gr.Textbox(
                label='Training models count',
                placeholder='Count of folders that will be created',
                value=1,
                interactive=True
            )
        with gr.Row():
